...
__global__ void reduceNomal(float *d_input, float *d_output)
{
    ...
    unsigned int global_id = threadIdx.x + (blockDim.x*2)*blockIdx.x;
    sData[thread_id] = d_input[global_id] + d_input[global_id + blockDim.x];
    __syncthreads();
...
int main()
{
    ...
    int num_block = ceil((float)N/NUM_PER_BLOCK);
